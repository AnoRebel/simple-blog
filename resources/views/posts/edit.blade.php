  ...
  <div class="col-lg-8 col-md-8 col-sm-10">
    <form method="POST" action="{{ route('posts.update', [$post->id]) }}">
      ...
      <div class="form-group">
        ...
        <input type="text" class="form-control{{ $errors->has('title') ? ' is-invalid' : '' }} bg-transparent" name="title" id="title" placeholder="Enter Title" value="{{ $post->title }}" required="" autofocus="">
        @if ($errors->has('title'))
      ...
      <div class="form-group">
        ...
        <textarea class="form-control{{ $errors->has('body') ? ' is-invalid' : '' }} bg-transparent" name="body" id="body" rows="6"> {{ $post->body }} </textarea>
        @if ($errors->has('body'))
            <span class="invalid-feedback" role="alert">
